...
def smoothness_loss(Delta: torch.Tensor) -> torch.Tensor:
    """
    Penalize the second-order finite differences of the perturbation.
    Encourages smooth, gradual changes rather than discontinuous jumps.
    Delta shape: (N, T, D)
    """
    # 2nd derivative: Delta[t+1] - 2*Delta[t] + Delta[t-1]
