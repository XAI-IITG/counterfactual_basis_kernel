...
from .CMAPSSDataLoader import CMAPSSDataLoader
from .CMAPSSDatasetWrapper import CMAPSSDatasetWrapper
from .CMAPSSTimeSeriesDataset import CMAPSSTimeSeriesDataset
...
class CMAPSSTorchDataloader:
    def __init__(self, data_path: str = None):
        pass

    def split_units(self, unit_ids: np.ndarray, val_ratio: float = 0.2, seed: int = 42) -> Tuple[Set[int], Set[int]]:
    ...
    def add_test_rul_rowwise(self, test_df, rul_true, max_rul: int):
    ...
    def remove_low_variance(self, train_df, val_df, test_df, feature_cols: List[str], threshold: float):
    ...
    def standardise(self, train_df, val_df, test_df, feature_cols: List[str]):
    ...
    def make_last_window_arrays(self, df_norm, seq_len: int, feature_cols: List[str]):
        Xs, ys = [], []
    ...
    def load_and_preprocess_data(self,
        ...
        test_df = self.add_test_rul_rowwise(test_df, rul_true, max_rul=max_rul)

        train_units, val_units = self.split_units(train_df["unit_id"].unique(), val_ratio=val_ratio, seed=seed)
        ...
        kept_features = self.remove_low_variance(train_split, val_split, test_df, feature_cols_all, threshold=var_threshold)
        ...
        train_norm, val_norm, test_norm, mean, std = self.standardise(train_split, val_split, test_df, kept_features)
        ...
        Xt, yt = self.make_last_window_arrays(test_norm, seq_len, kept_features)
